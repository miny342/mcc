...
assert(t60, {
    ...
    a.b = 1;
    a.d.c = 5;
    return b->a[0] + b->a[1] + b->a[2] + b->b + b->d.c;
}, 14);
...
assert(t65, {return _t65.val + _t65.s + _t65.e;}, 14);
assert(t66, {
    int i = 0;
    int j = 0;
    do {
...
int _t68(int a, int b, int c, int d, int e, int f, int g, ...) {
    va_list ap;
    va_start(ap, g);
    return a + b + vfprintf(stdout, "%d %d %d\n", ap);
}
assert(t68, {
